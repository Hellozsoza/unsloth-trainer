...
if command -v uv &>/dev/null; then
    USE_UV=1
    PIP_CMD="uv pip"
    VENV_CREATE_CMD="uv venv"
    echo "⚡ uv detected — using uv for faster installs."
else
    USE_UV=0
    PIP_CMD="pip"
...
# uv pip nem igényel upgrade-et, csak pip esetén
if [ "$USE_UV" -eq 0 ]; then
...
$PIP_CMD install torch torchvision torchaudio --index-url "$TORCH_INDEX" --no-cache-dir
if [ $? -ne 0 ]; then
    echo "❌ PyTorch install failed. Aborting."
    exit 1
fi
echo "🧹 Clearing cache after Step 1..."
clear_pip_cache
...
if [ -n "$UNSLOTH_EXTRA" ]; then
    $PIP_CMD install "unsloth[$UNSLOTH_EXTRA] @ git+https://github.com/unslothai/unsloth.git" --no-cache-dir
    if [ $? -ne 0 ]; then
        echo "⚠️  Versioned Unsloth install failed — trying generic install..."
        $PIP_CMD install unsloth --no-cache-dir
    fi
else
    $PIP_CMD install unsloth --no-cache-dir
fi
...
echo "🧹 Clearing cache after Step 2..."
clear_pip_cache
...
if [ -n "$LLAMA_CMAKE" ]; then
    CMAKE_ARGS="$LLAMA_CMAKE" $PIP_CMD install llama-cpp-python --no-cache-dir
    if [ $? -ne 0 ]; then
        echo "⚠️  CUDA build failed — falling back to CPU-only llama-cpp-python"
        $PIP_CMD install llama-cpp-python --no-cache-dir
        if [ $? -ne 0 ]; then
...
echo "🧹 Clearing cache after Step 3..."
clear_pip_cache
...
if [ "$SKIP_FLASH" -eq 0 ]; then
    $PIP_CMD install wheel ninja --quiet --no-cache-dir
    echo "   Building flash-attn with MAX_JOBS=2 (this can take 10-30 min)..."
    (MAX_JOBS=2 $PIP_CMD install flash-attn --no-build-isolation --no-cache-dir)
    if [ $? -ne 0 ]; then
...
echo "🧹 Clearing cache after Step 4..."
clear_pip_cache
...
echo "🧹 Clearing cache after Step 5..."
clear_pip_cache
