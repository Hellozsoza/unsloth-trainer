    ...
    try:
        if TOKEN_FILE.exists():
            ...
            if token:
                ...
                print(
                    f"[INFO] Auto-logged in to HuggingFace as: {hf_token['username']}"
                )
...
SYNTH_TOPICS = {
    ...
    "reasoning": [
        "a train leaves at 60mph and another at 80mph",
        "if all cats are animals and Felix is a cat",
        "prove sqrt(2) is irrational",
        "a farmer has chickens and cows totaling 20 heads and 56 legs",
    ],
    "general": [
        "photosynthesis",
                ...
                while True:
                    ...
                    if total > 0:
                        ...
                        if pct != last_pct and pct % 10 == 0:
                            emit_log(
                                f"  Downloading {filename}: {pct}% ({downloaded // 1024 // 1024}MB / {total // 1024 // 1024}MB)",
                                "info",
                            )
                            last_pct = pct
        tmp.rename(local_path)
        emit_log(
            f"Downloaded: {local_path} ({downloaded // 1024 // 1024} MB)", "success"
        )
        return str(local_path)
    except Exception as e:
        if tmp.exists():
...
def run_autodistill(config):
    ...
    try:
        ...
        emit_log(
            f"Teacher: {preset['teacher_file']} ({preset['teacher_params']})", "info"
        )
        ...
        set_progress(2)
        teacher_path = _download_gguf_teacher(
            preset["teacher_repo"], preset["teacher_file"]
        )
        set_progress(8)
        ...
        if is_tool_calling:
            ...
            prompts = [
                f"{p} {formats[i % len(formats)]}" for i, p in enumerate(raw_topics)
            ]
        ...
        pairs = []
        emit_log(
            f"Generating {num_prompts} teacher responses (GGUF, ~10 tok/s) ...", "info"
        )
        ...
        del pairs
        gc.collect()  # free the raw list before loading the student model
        set_progress(52)
        ...
        if use_loftq:
            load_4bit = False
            emit_log(
                f"LoftQ enabled for student — loading in full precision ({loftq_bits}-bit, {loftq_iter} iter)",
                "info",
            )
...
def _load_dataset_hf(
    ...
    if streaming:
        emit_log(
            f"Dataset streaming ON — will materialise first {max_samples} rows", "info"
        )
        kw["streaming"] = True
...
def _check_not_gguf(model_name):
    if "gguf" in str(model_name).lower():
        raise ValueError(
            f"'{model_name}' is a GGUF model (inference-only). Use the safetensors version for training."
        )
    p = Path(model_name)
    if p.is_dir() and (list(p.glob("*.gguf")) + list(p.glob("*.GGUF"))):
        raise ValueError(
            f"Directory '{model_name}' contains GGUF files — cannot fine-tune."
        )
...
def load_model_and_tokenizer(model_name, max_seq_length=2048, load_in_4bit=True):
    ...
    _resolved = (
        str(Path(model_name).resolve()) if Path(model_name).exists() else model_name
    )
    ...
    if _cfg_path.exists():
        try:
            ...
            if not _cfg.get("model_type"):
                ...
                emit_log(
                    f"Patched config.json: added model_type='{_inferred}' (from architecture '{_arch}')",
                    "warn",
                )
    ...
    try:
        ...
        emit_log(
            f"VRAM used: {round(torch.cuda.memory_allocated() / 1024**3, 2)} GB", "info"
        )
        _active_model["model"] = model
        _active_model["tokenizer"] = tokenizer
        return model, tokenizer, "unsloth_fastmodel"
    except Exception as e:
        errors["FastModel"] = str(e)
        emit_log(f"FastModel failed: {e}", "warn")
    ...
    try:
        ...
        errors["FastLanguageModel"] = str(e)
        emit_log(f"FastLanguageModel also failed: {e}", "warn")
    ...
    def _sanitize_local_configs(path):
        ...
        def _patch_file(file_path, removals=(), dtype_fields=()):
            ...
            if changed:
                ...
                emit_log(
                    f"Sanitized {file_path.name} for transformers fallback", "info"
                )
                restores.append((file_path, original))

        _patch_file(model_dir / "config.json", removals=("quantization_config",))
        _patch_file(
    ...
    raise RuntimeError(
        f"All loading methods failed for '{model_name}'.\n{error_summary}"
    )
...
def apply_lora(
    model, r=16, alpha=16, dropout=0.0, use_loftq=False, loftq_bits=4, loftq_iter=1
):
    """Apply LoRA — auto-detects MoE expert projection layers (w1/w2/w3).
    When use_loftq=True, uses LoftQ initialization via PEFT for better quantized training."""
    target_modules = _get_lora_target_modules(model)

    if use_loftq:
        from peft import get_peft_model, LoraConfig, TaskType, LoftQConfig

        loftq_config = LoftQConfig(bits=loftq_bits, iter=loftq_iter)
        ...
        emit_log(
            f"Applied LoRA with LoftQ ({loftq_bits}-bit, {loftq_iter} iter)", "success"
        )
        return model
    ...
    try:
        ...
        emit_log(
            f"FastLanguageModel LoRA failed ({e2}), falling back to PEFT...", "warn"
        )
...
def run_sft(model, tokenizer, dataset, config, progress_start=40, progress_end=90):
    ...
    if low_memory:
        ...
        emit_log(
            f"Low-memory mode ON — seq_length capped to {max_seq}, batch=1, grad_accum=8",
            "info",
        )
        effective_batch_size = cfg_int(config.get("batch_size"), 2) * cfg_int(
            config.get("grad_accum"), 4
        )
        _batch_size = 1
        _grad_accum = max(
            effective_batch_size, 8
        )  # preserve effective batch, minimum 8
    else:
    ...
    if speed_mode:
        emit_log(
            "Speed mode ON — tf32, fused optimizer, packing, no evals/checkpoints",
            "info",
        )
    ...
    def _row_to_text(row):
        ...
        if isinstance(raw, list) and len(raw) > 0:
            ...
            for item in raw:
                if isinstance(item, dict):
                    ...
                    content_val = item.get(
                        "content", item.get("value", item.get("text", ""))
                    )
                    parts.append(
                        f"### {role.capitalize()}:\n{content_val}"
                        if role
        ...
        inp = row.get(
            "input", row.get("context", row.get("question", row.get("problem", "")))
        )
        output = row.get(
            "output", row.get("response", row.get("answer", row.get("solution", "")))
        )
    ...
    try:
        ...
        emit_log(
            f"Formatting test OK: {len(test[0])} chars. Preview: {test[0][:200]}",
            "success",
        )
    except Exception as e:
    ...
    _auto_steps = (
        str(_raw_steps_val).strip().lower() in ("auto", "0", "") or _raw_steps_val == 0
    )
    if _auto_steps:
        _max_steps = 5000  # hard safety cap; early-stop callback halts sooner
        emit_log(
            "Auto-steps mode ON — will train until loss ≤ 2.00, then round up to nearest ×50",
            "info",
        )
    else:
    ...
    if speed_mode:
        ...
        emit_log(
            f"Speed mode: adamw_8bit, tf32={_tf32}, group_by_length, {_dl_workers} dataloader workers",
            "info",
        )
    else:
    ...
    class ProgressCB(TrainerCallback):
        def on_log(self, args, state, control, logs=None, **kw):
            if state.global_step:
                pct = progress_start + (state.global_step / total) * (
                ...
                loss_str = (
                    ...
                    else (str(loss_val) if loss_val is not None else "?")
                )
                tps = logs.get("train_samples_per_second", "")
                spd = f" | {tps:.1f} samples/s" if tps else ""
                emit_log(
                    f"Step {state.global_step}/{total} | loss: {loss_str}{spd}", "info"
                )
                ...
                if _auto_steps and _auto_steps_target[0] is None:
                    if isinstance(loss_val, float) and loss_val <= 2.00:
                        ...
                        emit_log(
                            ...
                            f" → rounding up to {rounded} steps",
                            "info",
                        )
        ...
        def on_step_end(self, args, state, control, **kw):
            ...
            if _auto_steps and _auto_steps_target[0] is not None:
                if state.global_step >= _auto_steps_target[0]:
                    emit_log(
                        f"Auto-steps: reached target {_auto_steps_target[0]} steps — stopping.",
                        "success",
                    )
    ...
    try:
        ...
        _actual_runtime = next(
            (
                e.get("train_runtime")
                for e in reversed(_state.log_history or [])
        ...
        if _actual_runtime and _actual_steps > 0:
            ...
            _tok_per_sec = next(
                (
                    e.get("train_tokens_per_second")
                    or e.get("train_samples_per_second")
                    for e in reversed(_state.log_history or [])
                    if "train_tokens_per_second" in e or "train_samples_per_second" in e
                ),
                None,
            )
            _entry = {
            ...
            emit_log(
                f"Timing logged: {round(_secs_per_step, 2)} s/it over {_actual_steps} steps",
                "info",
            )
        else:
...
def generate_text(
    model, tokenizer, prompt, max_new_tokens=512, temperature=0.7, top_p=0.9
):
...
def run_training(config):
    try:
        ...
        if use_loftq:
            load_4bit = False
            emit_log(
                f"LoftQ enabled — loading model in full precision ({loftq_bits}-bit, {loftq_iter} iter)",
                "info",
            )
        set_stage("Loading model")
        model, tok, _ = load_model_and_tokenizer(
            config["model_name"], cfg_int(config.get("max_seq_length"), 2048), load_4bit
        )
        set_progress(20)
        set_stage("Applying LoRA")
        model = apply_lora(
            model,
            r=cfg_int(config.get("lora_r"), 16),
            alpha=cfg_int(config.get("lora_alpha"), 16),
            dropout=cfg_float(config.get("lora_dropout"), 0.0),
            use_loftq=use_loftq,
            loftq_bits=loftq_bits,
            loftq_iter=loftq_iter,
        )
...
def run_multimodal_upgrade(config, preset):
    try:
        ...
        vision_enc.eval()
        vision_dim = vision_enc.config.hidden_size
        emit_log(f"Vision encoder loaded. dim={vision_dim}", "success")
        set_progress(15)
        ...
        emit_log(f"LLM loaded. dim={text_dim}", "success")
        set_progress(25)
        ...
        emit_log(
            f"Projection: {sum(p.numel() for p in proj.parameters()):,} params",
            "success",
        )
        set_progress(30)
        set_stage("Loading pre-training dataset")
        ds_info = preset["datasets"][0]
        kw = {"split": ds_info["split"]}
        if token:
            kw["token"] = token
        try:
        ...
        optimizer = torch.optim.AdamW(
            proj.parameters(), lr=cfg_float(config.get("learning_rate"), 1e-3)
        )
        ...
        step = 0
        running_loss = 0.0
        text_col = ds_info.get("text_col", "conversations")
        for i, row in enumerate(dataset):
            ...
            try:
                ...
                with torch.inference_mode():
                    ...
                    dummy = torch.zeros(
                        1, vision_dim, device=llm.device, dtype=torch.float16
                    )
                ...
                if step % 10 == 0:
                    emit_log(
                        f"Step {step}/{max_steps} | Loss: {running_loss / 10:.4f}",
                        "info",
                    )
                    running_loss = 0.0
                    set_progress(38 + (step / max_steps) * 45)
            except:
                continue
        emit_log("Stage 1 complete.", "success")
        set_progress(85)
        set_stage("Saving")
        ...
        with open(os.path.join(out_dir, "multimodal_config.json"), "w") as f:
            _json.dump(
                {
                    "base_llm": config["model_name"],
                    "vision_encoder": vision_repo,
...
def run_autotrain(config):
    ...
    try:
                ...
                if text_col not in dataset.column_names:
                    for c in [
                        ...
                        if c in dataset.column_names:
                            ...
                            break
                emit_log(
                    f"Dataset ready: {ds_info['repo']} ({len(dataset)} rows, col='{text_col}')",
                    "success",
                )
                break
            except Exception as e:
        ...
        if dataset is None:
            raise RuntimeError("All datasets failed to download.")
        set_progress(40)
        set_stage("Training")
...
def run_distillation(config):
    try:
        ...
        custom_prompts = (
            [p.strip() for p in _cp if p.strip()]
        ...
        if teacher_source == "cloud":
            ...
            for i, prompt in enumerate(prompts):
                ...
                try:
                    ...
                    with open(log_path, "a") as lf:
                        lf.write(
                            json.dumps(
                                {"_error": str(e), "prompt_index": i, "prompt": prompt}
                            )
                            + "\n"
                        )
                    raise
                ...
                entry = {
                    ...
                    "prompt": prompt,
                    "response": resp,
                    "tokens_in_est": tok_in,
                    "tokens_out_est": tok_out,
                ...
                set_progress(pct)
                if (i + 1) % 10 == 0:
                    cost = (total_in / 1_000_000) * price_in + (
                        total_out / 1_000_000
                    ) * price_out
                    emit_log(
                        f"Generated {i + 1}/{num_prompts} | ~{total_in // 1000}K in / {total_out // 1000}K out | cost so far: ${cost:.4f}",
                        "info",
                    )

            # Write summary to log
            final_cost = (total_in / 1_000_000) * price_in + (
                total_out / 1_000_000
            ) * price_out
            with open(log_path, "a") as lf:
                lf.write(
            ...
            emit_log(
                f"Cloud log saved: {log_path.name} | Total cost: ${final_cost:.4f}",
                "success",
            )
            ...
            if is_gguf_teacher:
                emit_log(
                    "GGUF teacher detected — loading via llama-cpp-python (inference only)",
                    "info",
                )
            ...
            emit_log(
                f"Inference batch size: {inf_batch} (adjust with inf_batch_size config)",
                "info",
            )
            for batch_start in range(0, num_prompts, inf_batch):
                ...
                if is_gguf_teacher:
                    resps = [
                        generate_text_gguf(llm_gguf, p, max_new_tokens=max_tokens)
                        for p in batch_prompts
                    ]
                else:
                    resps = generate_text_batched(
                        model,
                        tok,
                        batch_prompts,
        ...
        model, tok, _ = load_model_and_tokenizer(student_name, load_in_4bit=load_4bit)
        set_progress(55)
        model = apply_lora(
            model,
            r=cfg_int(config.get("lora_r"), 16),
...
def run_generate_dataset(config):
    try:
        ...
        custom_seeds = (
            [p.strip() for p in _sr if p.strip()]
        ...
        seeds = (
            custom_seeds
            if custom_seeds
            else [TEMPLATES.get(style, TEMPLATES["instruction"])]
        )
        ...
        if gen_source == "cloud":
            ...
            for i in range(num_samples):
                ...
                try:
                    ...
                    with open(log_path, "a") as lf:
                        lf.write(
                            json.dumps({"_error": str(e), "index": i, "prompt": seed})
                            + "\n"
                        )
                    raise
                ...
                if (i + 1) % 10 == 0:
                    ...
                    emit_log(
                        f"Generated {i + 1}/{num_samples} | ~{total_in // 1000}K in / {total_out // 1000}K out | cost: ${cost:.4f}",
                        "info",
                    )
            ...
            if not model_name:
                raise ValueError("model_name required for local generation")
            is_gguf_gen = _is_gguf_path(model_name)
            if is_gguf_gen:
                set_stage("Loading GGUF generator model")
                emit_log(
                    "GGUF generator detected — loading via llama-cpp-python (inference only)",
                    "info",
                )
            ...
            all_seeds = [
                seeds[i % len(seeds)].replace("{topic}", topic)
                for i in range(num_samples)
            ]
            for batch_start in range(0, num_samples, inf_batch):
                ...
                if is_gguf_gen:
                    outputs = [
                        generate_text_gguf(llm_gguf, s, max_new_tokens=max_tokens)
                        for s in batch_seeds
                    ]
                else:
                    outputs = generate_text_batched(
                        model,
                        tok,
                        batch_seeds,
                        max_new_tokens=max_tokens,
...
def _web_search(query, engine="duckduckgo", api_key="", num_results=5):
    ...
    if engine == "serpapi" and api_key:
        ...
        url = (
            ...
            + urllib.parse.urlencode({"q": query, "count": num_results})
        )
        req = urllib.request.Request(
            url,
            headers={
...
def _fetch_page_text(url, max_chars=8000):
    ...
    try:
        ...
        raw = re.sub(
            r"(?s)<(script|style|nav|header|footer|aside)[^>]*>.*?</\1>", " ", raw
        )
        raw = re.sub(r"<[^>]+>", " ", raw)
        raw = re.sub(r"&nbsp;", " ", raw)
...
def run_websearch_dataset(config):
    ...
    try:
        ...
        custom_queries = [
            q.strip() for q in config.get("custom_queries", []) if str(q).strip()
        ]
        ...
        if custom_queries:
            ...
            emit_log(
                f"Auto-generated {len(queries)} search queries for: {topic}", "info"
            )
        ...
        if ai_source == "cloud":
            emit_log(
                f"Using cloud AI: {config.get('cloud_provider')} / {config.get('cloud_model')}",
                "info",
            )
        else:
            model_name = config.get("model_name", "")
        ...
        for qi, query in enumerate(queries):
            ...
            for res in results:
                ...
                if text.strip():
                    all_contexts.append(
                        {"url": url, "title": title, "text": text, "query": query}
                    )
        ...
        if not all_contexts:
            raise RuntimeError(
                "No content retrieved from web search. Try a different topic or search engine."
            )
        ...
        for ci, ctx in enumerate(all_contexts):
            ...
            for chunk in chunks[:2]:  # max 2 chunks per source to stay efficient
                ...
                prompt = style_prompt_template.format(
                    text=chunk.strip(), n=pairs_per_chunk
                )
                ...
                try:
                    if ai_source == "cloud":
                        ...
                        raw = generate_text_gguf(
                            llm_gguf, prompt, max_new_tokens=max_tokens
                        )
                    else:
                        raw = generate_text(
                            model, tok, prompt, max_new_tokens=max_tokens
                        )
            ...
            if (ci + 1) % 5 == 0:
                emit_log(
                    f"Progress: {len(samples)} pairs so far from {ci + 1}/{total_ctx} sources",
                    "info",
                )
        ...
        if not samples:
            raise RuntimeError(
                "No dataset pairs were generated. The AI may have returned invalid JSON — try a different model or style."
            )
        ...
        summary = {
            ...
            "queries": queries,
            "sources_fetched": len(all_contexts),
        ...
        current_job["status"] = "done"
        emit_log(
            f"✅ Web dataset complete! {len(samples)} pairs from {len(all_contexts)} sources → {ds_name}",
            "success",
        )
...
def run_evaluation(config):
    try:
        ...
        try:
            ...
            dataset = load_dataset(ds_path, split="train", token=hf_token["value"])
        emit_log(f"Dataset loaded: {len(dataset)} rows", "success")
        ...
        for i, row in enumerate(samples):
            ...
            if expected:
                ...
                if exp_tokens:
                    score = round(
                        len(exp_tokens & gen_tokens) / len(exp_tokens) * 100, 1
                    )
                    scores.append(score)
            ...
            set_progress(pct)
            emit_log(
                f"Sample {i + 1}/{len(samples)} | score: {score}%",
                "info" if score is None or score >= 30 else "warn",
            )
        ...
        if avg_score is not None:
            ...
            emit_log(
                f"{emoji} Avg token overlap score: {avg_score}% over {len(scores)} samples",
                "success" if avg_score >= 50 else "warn",
            )
        else:
            emit_log(
                f"✅ Generated {len(results)} samples (no target col for scoring)",
                "success",
            )
    ...
    if provider == "google":
        try:
            ...
            for m in raw.get("models", []):
                ...
                supported = [
                    a.get("name", "") for a in m.get("supportedGenerationMethods", [])
                ]
                if "generateContent" in supported and any(
                    x in name for x in ["gemini", "gemma"]
                ):
                    models.append(name)
    ...
    return jsonify(
        {
            ...
            "current_job": {
                k: v if k != "logs" else f"{len(v)} entries"
...
def train():
    return _start_job(run_training, request.json)


@app.route("/api/autotrain", methods=["POST"])
def autotrain():
    return _start_job(run_autotrain, request.json)


@app.route("/api/distill", methods=["POST"])
def distill():
    return _start_job(run_distillation, request.json)


@app.route("/api/generate_dataset", methods=["POST"])
def gen_dataset():
    return _start_job(run_generate_dataset, request.json)


@app.route("/api/websearch_dataset", methods=["POST"])
def websearch_dataset():
    return _start_job(run_websearch_dataset, request.json)


@app.route("/api/autodistill", methods=["POST"])
def autodistill():
    return _start_job(run_autodistill, request.json)
    ...
    for path in list(_download_dirs):
        try:
            ...
            try:
                ...
                emit_log(
                    f"Stop: skipping {p.name} (in outputs/, not a download)", "info"
                )
                continue
            except ValueError:
                pass
            ...
            has_incomplete = any(
                f.suffix in (".incomplete", ".tmp") for f in p.rglob("*") if f.is_file()
            )
            if has_model and not has_incomplete:
                emit_log(
                    f"Stop: keeping {p.name} — looks complete (has model files)", "info"
                )
                continue
            shutil.rmtree(str(p))
...
def gguf_export():
    return _start_job(run_gguf_export, request.json)


@app.route("/api/mobile_export", methods=["POST"])
def mobile_export():
    return _start_job(run_mobile_export, request.json)


@app.route("/api/optimize", methods=["POST"])
def optimize():
    return _start_job(run_optimization, request.json)


@app.route("/api/prune", methods=["POST"])
def prune():
    return _start_job(run_pruning, request.json)
...
def evaluate():
    return _start_job(run_evaluation, request.json)
...
def list_local_models():
    ...
    def model_info(p, tag="model"):
        is_gguf = (
            bool(list(p.glob("*.gguf")) + list(p.glob("*.GGUF")))
            if p.is_dir()
            else str(p).lower().endswith(".gguf")
        )
        has_weights = (
            bool(list(p.glob("*.safetensors")) + list(p.glob("pytorch_model*.bin")))
            if p.is_dir()
            else False
        )
        try:
        ...
        return {
            ...
            "size": size_str,
            "trainable": has_weights and not is_gguf,
        }

    for p in sorted(MODELS_DIR.iterdir()):
        if p.is_dir():
            models.append(model_info(p, "model"))
        elif p.suffix.lower() == ".gguf":
            models.append(model_info(p, "gguf"))
    for p in sorted(OUTPUTS_DIR.iterdir()):
        if p.is_dir():
            models.append(model_info(p, "output"))
...
def list_hf_files():
    ...
    try:
        ...
        config_bytes = sum(
            ...
            and file_sizes.get(f, 0) < 10_000_000
        )
    ...
    try:
        ...
        try:
            for item in list_repo_tree(
                repo_id, repo_type="dataset", recursive=True, token=token
            ):
                size = getattr(item, "size", 0) or 0
                total_bytes += size
            ...
            if hasattr(raw_splits, "items"):
                for name, si in raw_splits.items():
                    ...
                    if n is not None and name in splits:
                        ...
                        splits.setdefault(
                            name, {"size": 0, "num_rows": int(n), "files": []}
                        )
                        splits[name]["num_rows"] = int(n)
        except Exception:
            pass
...
def download_model():
    ...
    def do(repo_id, patterns, filename, filenames, out_name):
        ...
        current_job.clear()
        current_job.update(
            status="downloading", progress=0, logs=[], stage="Connecting to HuggingFace"
        )
            ...
            while not monitor_stop.is_set():
                try:
                    ...
                    if expected_bytes > 0:
                        ...
                        current_job["stage"] = (
                            f"{done_mb:.0f} / {expected_bytes / 1e6:.0f} MB ({pct}%)"
                        )
                    else:
                        current_job["stage"] = f"{done_mb:.0f} MB downloaded"
                        current_job["progress"] = min(
                            current_job.get("progress", 0) + 1, 90
                        )
                except Exception:
                    pass
        ...
        try:
            ...
            if filenames:
                ...
                emit_log(
                    f"Downloading {len(filenames)} selected file(s) from {repo_id}",
                    "info",
                )
                for fn in filenames:
                ...
                emit_log(
                    f"Downloading {len(patterns)} pattern(s) from {repo_id}", "info"
                )
    ...
    t = threading.Thread(
        target=do, args=(repo_id, patterns, filename, filenames, out_name), daemon=True
    )
...
def import_gguf():
    ...
    name = request.form.get("alias", "").strip() or f.filename
    name = secure_filename(name)
    if not name:
        return jsonify({"ok": False, "error": "Could not determine filename"})
    if not name.lower().endswith(".gguf"):
        return jsonify({"ok": False, "error": "Only .gguf files supported"})
...
def delete_model():
    ...
    if not any(
        str(target).startswith(str(a.resolve())) for a in [MODELS_DIR, OUTPUTS_DIR]
    ):
    ...
    try:
        ...
        emit_log(f"Deleted model: {target.name}", "warn")
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)})
...
def list_local_datasets():
    ...
    if DATASETS_DIR.exists():
        for p in DATASETS_DIR.iterdir():
            results.append(
                {"name": p.name, "path": str(p), "source": "datasets", "size": sz(p)}
            )
    if GEN_DIR.exists():
        for p in GEN_DIR.glob("*.jsonl"):
            results.append(
                {"name": p.name, "path": str(p), "source": "generated", "size": sz(p)}
            )
...
def delete_dataset():
    ...
    target = Path(path).resolve()
    if not any(
        str(target).startswith(str(a.resolve())) for a in [DATASETS_DIR, GEN_DIR]
    ):
        return jsonify({"ok": False, "error": "Path outside allowed directories"})
    if not target.exists():
        return jsonify({"ok": False, "error": "Path does not exist"})
    try:
        shutil.rmtree(str(target)) if target.is_dir() else target.unlink()
        emit_log(f"Deleted dataset: {target.name}", "warn")
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)})
        ...
        def _pulse():
            while not pulse_stop.is_set():
                ...
                if p < 85:
                    ...
                    current_job["stage"] = (
                        f"↓ Downloading dataset: {repo_id.split('/')[-1]}…"
                    )
                pulse_stop.wait(2.0)
...
def list_outputs():
    return jsonify(
        [{"name": p.name, "path": str(p)} for p in OUTPUTS_DIR.iterdir() if p.is_dir()]
    )
    ...
    for k, v in AUTOTRAIN_PRESETS.items():
        result[k] = {
            ...
            "recommended_datasets": [
                {"repo": d["repo"], "split": d["split"]} for d in v["datasets"]
            ],
            "lora_r": v["lora_r"],
            "max_steps": v["max_steps"],
            "projection_only": v.get("projection_only", False),
            "vision_encoder": v.get("vision_encoder", None),
        }
    return jsonify(result)
...
def hf_logout():
    ...
    hf_token["value"] = None
    hf_token["username"] = None
    if delete_saved and TOKEN_FILE.exists():
        TOKEN_FILE.unlink()
        emit_log("Saved token deleted.", "info")
    emit_log("Logged out of HuggingFace.", "info")
    return jsonify({"ok": True})
...
def hf_status():
    return jsonify(
        {
            "logged_in": hf_token["value"] is not None,
...
def api_tuning_stats():
    ...
    if not entries:
        return jsonify(
            {"ok": True, "count": 0, "secs_per_step": None, "tok_per_sec": None}
        )
    ...
    sps_vals = sorted(
        e["secs_per_step"]
        for e in entries
        if "secs_per_step" in e and e["secs_per_step"] > 0
    )
    tps_vals = sorted(
        e["tok_per_sec"] for e in entries if "tok_per_sec" in e and e["tok_per_sec"] > 0
    )
    ...
    try:
        ...
        if not patches:
            return jsonify(
                {
                    "ok": False,
                    "error": "Nothing to patch — all fields already set or no values provided",
                }
            )
...
def run_rope_scale(config):
    try:
        ...
        out_name = (
            ...
            or f"rope_{rope_type}_{int(factor)}x_{int(time.time())}"
        )
        ...
        if rope_type in ("yarn", "llama3"):
            ...
            rope_scaling["attention_factor"] = float(
                config.get("attention_factor", 0.1)
            )
...
def run_create_blank_model(config):
    try:
        ...
        set_stage("Building model config")
        set_progress(5)
        ...
        emit_log(
            f"Config: {model_type}, layers={num_layers}, hidden={hidden_size}, heads={num_heads}, max_pos={max_pos}",
            "info",
        )
        set_progress(15)
        # ── GPU / CPU init ────────────────────────────────────────────────────
        ...
        if use_gpu and torch.cuda.is_available():
            ...
            emit_log(
                f"GPU: {gpu_name} — {vram_free} GB free — init on GPU (fp16)", "success"
            )
        else:
            device = torch.device("cpu")
            emit_log(
                "No CUDA GPU found — using CPU (fp32)"
                if use_gpu
                else "CPU mode (fp32)",
                "warn" if use_gpu else "info",
            )
        ...
        emit_log("Tokenizer built", "success")
        set_progress(65)
        ...
        return jsonify(
            {
                "ok": True,
                "vram_gb": vram_gb,
                "fits": vram_gb <= 12.0 or vram_gb == 0.0,
                "limit_gb": 12.0,
            }
        )
    except Exception as e:
...
def chat_load():
    ...
    if "gguf" in model_path.lower():
        return jsonify(
            {"ok": False, "error": "GGUF models not supported for chat here."}
        )
    vram_gb = _estimate_vram_gb(model_path, load_in_4bit)
    if vram_gb > 12.0:
        return jsonify(
            {
                "ok": False,
                "error": f"Estimated VRAM {vram_gb} GB > 12 GB limit. Use 4-bit or smaller model.",
            }
        )
        ...
        with chat_lock:
            ...
            try:
                ...
                model, tokenizer, loader = load_model_and_tokenizer(
                    model_path, max_seq_length=max_seq, load_in_4bit=load_in_4bit
                )
                try:
                    from unsloth import FastModel
                ...
                try:
                    ...
                    actual_vram = round(torch.cuda.memory_allocated() / 1e9, 2)
                except:
                    pass
                chat_model_state.update(
                    {
                        "model": model,
                        "tokenizer": tokenizer,
                ...
                chat_model_state["status"] = "error"
                chat_model_state["error"] = str(e)
                print(f"[ERROR] Chat load failed: {e}")

    chat_model_state["status"] = "loading"
    chat_model_state["error"] = None
            ...
            with torch.no_grad():
                ...
                for _ in range(max_new_tokens):
                    if stop_flag.is_set():
                        break
                    out = model(
                        input_ids=gen_ids, attention_mask=torch.ones_like(gen_ids)
                    )
                    logits = out.logits[:, -1, :]
                    if temperature > 0:
...
def run_gguf_export(config):
    try:
        ...
        try:
            ...
            for i, qt in enumerate(quant_types):
                ...
                try:
                    model.save_pretrained_gguf(
                        out_base / output_name, tok, quantization_method=qt.lower()
                    )
            ...
            emit_log(
                f"Unsloth GGUF export failed: {e}, trying llama.cpp fallback", "warn"
            )
            ...
            if not convert_py:
                raise RuntimeError(
                    "llama.cpp not found. Install llama-cpp-python or llama.cpp."
                )
            ...
            if quantize_bin:
                for i, qt in enumerate(quant_types):
                    if stop_flag.is_set():
                        raise KeyboardInterrupt()
                    if qt in ("F16", "BF16", "F32"):
                        results.append({"quant": qt, "file": f16_path, "ok": True})
                        continue
                    out_file = str(out_base / f"{output_name}_{qt}.gguf")
                    set_stage(f"Quantizing {qt}")
                    r2 = subprocess.run(
                        [quantize_bin, f16_path, out_file, qt],
                        capture_output=True,
                        text=True,
                        timeout=300,
                    )
                    if r2.returncode == 0:
                        ...
                        results.append(
                            {"quant": qt, "error": r2.stderr[-200:], "ok": False}
                        )
        ...
        emit_log(
            f"GGUF export done: {ok_count}/{len(results)} formats succeeded → {out_base}",
            "success",
        )
...
def _get_sentencepiece_tokenizer(model_path, out_dir):
    ...
    found_tools = _glob.glob(
        "/usr/local/lib/**/tokenizer_to_sentencepiece.py", recursive=True
    )
    ...
    for tool in tool_candidates:
        if Path(tool).exists():
            ...
            if r.returncode == 0 and Path(out_tok).exists():
                emit_log(
                    "Tokenizer converted to SentencePiece via ai_edge_torch", "info"
                )
                return out_tok
            else:
    ...
    try:
        ...
        emit_log(
            "No SentencePiece tokenizer.model found. For non-Gemma/Llama models, "
            "run: python -m ai_edge_torch.generative.tools.tokenizer_to_sentencepiece "
            f"--checkpoint {model_path} --output_path {sp_model}",
            "warn",
        )
    except ImportError:
        pass
...
def run_mobile_export(config):
    try:
        ...
        emit_log(
            f"Target: {target} | Model family: {family} | Quantize: {quantize}", "info"
        )
        ...
        if target == "litertlm":
            ...
            set_progress(8)
            emit_log(
                "This step is CPU-intensive and may take 10–60 min depending on model size",
                "warn",
            )
            emit_log(
                f"Quantization: {quantize} | Prefill: {prefill_len} | KV cache: {kv_len}",
                "info",
            )
            ...
            if converter_module:
                emit_log(
                    f"Using ai-edge-torch example converter: {converter_module}", "info"
                )
                convert_cmd = [
                    "python3",
                    "-m",
                ...
                r = subprocess.run(
                    convert_cmd, capture_output=True, text=True, timeout=7200
                )
                if r.returncode != 0:
                    emit_log(f"Example converter failed: {r.stderr[-600:]}", "warn")
                    emit_log(
                        "Falling back to generic ai_edge_torch conversion...", "info"
                    )
                    ...
                    if produced:
                        ...
                        emit_log(
                            f"TFLite produced: {Path(tflite_path).name}", "success"
                        )
            ...
            if not converter_module or not Path(tflite_path).exists():
                ...
                try:
                    ...
                    edge_model = ai_edge_torch.convert(
                        hf_model, sample_input, quant_config=quant
                    )
                    ...
                    raise RuntimeError(
                        ...
                        "ai-edge-torch and re-run with a supported architecture."
                    ) from eg
            ...
            if not tok_path:
                emit_log(
                    "WARNING: Could not find SentencePiece tokenizer.model. "
                    "Bundling will be skipped. Manually add tokenizer.model and re-run bundler.",
                    "warn",
                )
            set_progress(70)
            ...
            if tok_path:
                ...
                try:
                    ...
                    if Path(out_bundle).exists():
                        ...
                        emit_log(
                            f"✅ .litertlm bundle created: {Path(out_bundle).name} ({size_mb:.1f} MB)",
                            "success",
                        )
                    else:
                        ...
                        if Path(task_alt).exists():
                            ...
                            emit_log(
                                f"✅ Bundle created (renamed .task → .litertlm): {Path(out_bundle).name}",
                                "success",
                            )
                        else:
                            emit_log(
                                "Bundler ran but output not found — check out_dir for .task file",
                                "warn",
                            )
                except Exception as be:
                    emit_log(f"Bundler error: {be}", "error")
                    emit_log(
                        "TFLite file still available — manually bundle with mediapipe bundler.",
                        "warn",
                    )
            else:
                emit_log(
                    "Skipping bundling: no tokenizer. TFLite is at: " + tflite_path,
                    "warn",
                )
...
def run_optimization(config):
    try:
        ...
        if do_torch_compile:
            ...
            emit_log(
                "Compiling model graph — first inference will be slow, then fast",
                "info",
            )
            try:
            ...
            _json.dump(
                {
                    ...
                    "dynamic_quant": do_dyn_quant,
                    "kv_cache_quant": do_kv_quant,
                    "flash_attention": do_flash,
...
def _generate_concept_sentences(concept, advisor_source, advisor_config, n=40):
    ...
    if advisor_source == "local":
        ...
        lines = [
            l.strip().lstrip("0123456789.-) ") for l in raw.splitlines() if l.strip()
        ]
...
def _compute_concept_activations(model, tokenizer, sentences, top_k_rows=0.15):
    ...
    with torch.no_grad():
        for i, sent in enumerate(sentences):
            ...
            if (i + 1) % 10 == 0:
                emit_log(
                    f"  Activation probe: {i + 1}/{len(sentences)} sentences", "info"
                )
...
def run_pruning(config):
    try:
        ...
        threshold_pct = float(
            config.get("threshold_pct", 0.15)
        )  # top-% rows to zero per concept
        ...
        for name, module in model.named_modules():
            if not isinstance(module, torch.nn.Linear):
                continue
            if layer_filter and not any(
                f.strip() in name for f in layer_filter.split(",")
            ):
                continue
        ...
        if method == "magnitude":
            emit_log(
                f"Magnitude pruning: {len(targets)} layers at {sparsity * 100:.0f}% sparsity",
                "info",
            )
        ...
        # ══════════════════════════════════════════════════════════════════════
            ...
            head_targets = [
                ...
                for n, m in targets
                if any(
                    x in n for x in ["q_proj", "k_proj", "v_proj", "out_proj", "o_proj"]
                )
            ]
            emit_log(
                f"Attention head pruning: {len(head_targets)} projection layers", "info"
            )
        ...
        # ══════════════════════════════════════════════════════════════════════
            ...
            if not concepts:
                raise ValueError(
                    "No concepts specified. Enter at least one topic to erase."
                )

            concept_list = [
                c.strip()
                for c in concepts.replace(",", chr(10)).splitlines()
                if c.strip()
            ]
            emit_log(
                f"Concept erasure: {len(concept_list)} concept(s): {concept_list}",
                "info",
            )
            emit_log(
                f"Threshold: top {threshold_pct * 100:.0f}% concept-activating rows per layer",
                "info",
            )
            ...
            for ci, concept in enumerate(concept_list):
                ...
                stats = _zero_concept_rows(
                    model, row_scores, named_linears, threshold_pct, compress
                )
                emit_log(
                    f"  Zeroed {stats['zeroed_rows']} rows | "
        ...
        emit_log(
            f"Final sparsity: {zp / max(tp, 1) * 100:.1f}% zeros ({zp:,}/{tp:,})",
            "info",
        )
    ...
    for k in keys:
        ...
        for sd, wi in zip(sds, w):
            ...
            if flat.numel() > 1:
                ...
                delta = torch.where(
                    delta.abs() >= threshold, delta, torch.zeros_like(delta)
                )
...
def run_merge_models(config):
    try:
        ...
        weights_raw = config.get(
            "weights", []
        )  # list of floats (same length as models)
        base_path = config.get(
            "base_model", ""
        ).strip()  # required for task_arithmetic/ties/dare
        ...
        if len(set(arch_types)) > 1:
            emit_log(
                f"⚠️  Architecture mismatch: {arch_types}. Merge may fail or produce garbage.",
                "warn",
            )
        if len(set(vocab_sizes)) > 1:
            emit_log(
                f"⚠️  Vocab size mismatch: {vocab_sizes}. This will likely fail.", "warn"
            )
        else:
            emit_log(
                f"Architecture: {arch_types[0] if arch_types else 'unknown'} | "
                f"Vocab: {vocab_sizes[0] if vocab_sizes else '?'} | "
                f"Hidden: {hidden_sizes[0] if hidden_sizes else '?'}",
                "info",
            )
        ...
        for i, sd in enumerate(sds[1:], 1):
            ...
            if missing:
                emit_log(
                    f"Model {i + 1} missing {len(missing)} keys vs model 1 — shapes may differ",
                    "warn",
                )
            if extra:
                emit_log(
                    f"Model {i + 1} has {len(extra)} extra keys vs model 1 — will be ignored",
                    "warn",
                )
        ...
        if missing:
            emit_log(
                f"Missing keys after merge: {len(missing)} (may be fine for some architectures)",
                "warn",
            )
        if unexpected:
...
def run_abliterate(config):
    ...
    try:
        ...
        if norm < 1e-8:
            raise RuntimeError(
                "Refusal direction near-zero — try more calibration prompts."
            )
        refusal_dir = (refusal_dir / norm).to(model.device)
        emit_log(
            f"Refusal direction computed (pre-norm magnitude: {norm:.4f})", "success"
        )
        set_progress(60)

        set_stage("Projecting out refusal direction")
        layer_filters = (
            [f.strip() for f in layer_filter.split(",")] if layer_filter else []
        )
        n_modified = 0
        for param_name, param in model.named_parameters():
            ...
            W = param.data.float()
            param.data = (
                W - threshold * (W @ refusal_dir).unsqueeze(-1) * refusal_dir
            ).to(param.dtype)
            n_modified += 1
        emit_log(
            f"Projected out of {n_modified} matrices (threshold={threshold})", "success"
        )
        set_progress(85)
        ...
        emit_log(
            f"  {n_modified} matrices modified | threshold={threshold} | prompts={n_prompts}",
            "info",
        )
